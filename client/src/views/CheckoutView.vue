  ...
  <div class="min-h-screen flex flex-col bg-white">
    ...
    <main class="flex-grow container max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div class="text-center mb-8 sm:mb-12 mt-4">
        ...
        <p class="text-sm sm:text-base text-stone-600 max-w-2xl mx-auto">Укажите детали оплаты и получения товара</p>
      </div>
      ...
      <div v-if="!cartStore.cart?.products_in_cart?.length" class="text-center py-12">
        <div class="bg-white rounded-2xl shadow-lg border border-stone-300 p-12 max-w-md mx-auto">
          <div class="w-20 h-20 mx-auto mb-6 bg-gradient-to-br from-yellow-100 to-amber-100 rounded-full flex items-center justify-center">
            <svg class="w-10 h-10 text-yellow-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"/>
            </svg>
          </div>
          <h3 class="text-xl font-bold text-stone-800 mb-3">Корзина пуста</h3>
          <p class="text-stone-600 mb-6">Добавьте товары в корзину перед оформлением заказа</p>
          <router-link
              to="/chapters"
              class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-yellow-500 to-amber-500 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl transition-all duration-300"
          >
            Перейти в магазин
          </router-link>
        </div>
      ...
      <div v-else-if="cartStore.cart?.products_in_cart?.length" class="lg:grid lg:grid-cols-12 lg:gap-8">
        <div class="lg:col-span-7">
          <div class="bg-white rounded-2xl shadow-lg border border-stone-300 overflow-hidden lg:sticky lg:top-24">
            ...
            <form @submit.prevent="submitOrder" class="p-6 space-y-6">
              <div>
                ...
                <textarea
                    ...
                    class="w-full px-4 py-3 border border-stone-300 rounded-xl focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500 transition-all duration-300 resize-none"
              ...
              <div>
                ...
                <input
                    ...
                    class="w-full px-4 py-3 border border-stone-300 rounded-xl focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500 transition-all duration-300"
                    :class="{ 'border-red-500': errors.phone }"
                    required
                >
                <p v-if="errors.phone" class="mt-1 text-sm text-red-600">{{ errors.phone }}</p>
                <p class="mt-1 text-xs text-stone-500">Формат: +79991234567 или 89991234567</p>
              </div>

              <div>
                <label for="delivery_date" class="block text-sm font-medium text-stone-700 mb-2">
                  Дата доставки *
                </label>
                <input
                    id="delivery_date"
        ...
        <div class="mt-6 lg:mt-0 lg:col-span-5">
          <div class="bg-gradient-to-br from-white to-stone-50 rounded-2xl shadow-lg border border-stone-300 p-6 lg:sticky lg:top-8">
            <div class="flex items-center space-x-3 mb-6">
              ...
              <h2 class="text-xl font-bold text-stone-800">Ваш заказ</h2>
            </div>
            ...
            <div class="space-y-4 mb-6 max-h-96 overflow-y-auto">
              <div
                  ...
                  :key="item.id"
                  class="flex items-center space-x-4 p-4 bg-white rounded-xl border border-stone-200 shadow-sm"
              >
                <img
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <p class="text-xs text-stone-600 mt-1">Вариант: {{ item.product_variant.name }}</p>
            ...
            <dl class="space-y-4 border-t border-stone-300 pt-4">
              <div class="flex items-center justify-between">
                <dt class="text-base text-stone-600">Товары ({{ cartStore.cart.total_items }})</dt>
              ...
              <div class="flex items-center justify-between">
                <dt class="text-base text-stone-600">Доставка</dt>
                <dd class="font-medium text-green-600">Бесплатно</dd>
              </div>

              <div class="flex items-center justify-between pt-4 border-t border-stone-300">
                <dt class="text-lg font-bold text-stone-800">Общая стоимость</dt>
                <dd class="text-xl font-bold bg-gradient-to-r from-yellow-500 to-amber-500 bg-clip-text text-transparent">
                  {{ Intl.NumberFormat('ru-RU', { style: 'currency', currency: 'RUB' }).format(+cartStore.cart.total_price) }}
                </dd>
              </div>
            </dl>
            ...
            <div class="mt-6 space-y-4">
              ...
              <router-link
                  to="/cart"
                  class="w-full border-2 border-stone-300 text-stone-700 hover:border-yellow-300 hover:bg-yellow-50 font-medium py-3 px-6 rounded-xl transition-all duration-300 flex items-center justify-center space-x-2"
              >
